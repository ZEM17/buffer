...
def testing(epoch, nn_model, log_file):
    ...
    # run test script
    os.system('python test_daac.py ' + nn_model)
...
def central_agent(net_params_queues, exp_queues):
    ...
    with open(LOG_FILE + '_test.txt', 'w') as test_log_file:
        ...
        for epoch in range(TRAIN_EPOCH):
            ...
            s, a1, a2, a3, p1, p2, r, adv = [], [], [], [], [], [], [], []
            for i in range(NUM_AGENTS):
                s_, a1_, a2_, a3_, p1_, p2_, r_, adv_ = exp_queues[i].get()
                s += s_
                a1 += a1_
                a2 += a2_
                a3 += a3_
            ...
            a3_batch = np.vstack(a3)
            p1_batch = np.vstack(p1)
            p2_batch = np.vstack(p2)
            v_batch = np.vstack(r)
            adv_batch = np.vstack(adv)
            actor.train(s_batch, a1_batch, a2_batch, a3_batch, p1_batch, p2_batch, v_batch, adv_batch, epoch)
            ...
            if epoch % MODEL_SAVE_INTERVAL == 0:
                ...
                print("epoch:{}, qoe:{}, buffer:{}, maxbuf:{}, a/b core:{}".format(epoch, avg_reward, avg_buffer, avg_maxbuf, avg_reward / (avg_buffer/10)))
...
def agent(agent_id, net_params_queue, exp_queue):
    ...
    for epoch in range(TRAIN_EPOCH):
        obs = env.reset()
        s_batch, a1_batch, a2_batch, a3_batch, p1_batch, p2_batch, r_batch = [], [], [], [], [], [], []
        for step in range(TRAIN_SEQ_LEN):
            s_batch.append(obs)

            action1_prob, action2_prob, action3_adv = actor.predict(
            ...
            a3_batch.append(action3_adv)
            if done:
                break
        v_batch = actor.compute_v(s_batch, r_batch, done)
        gae_batch = actor.compute_gae(r_batch, s_batch)
        exp_queue.put([s_batch, a1_batch, a2_batch, p1_batch, p2_batch, v_batch, gae_batch, a3_batch])
