...
def test(NN_MODEL):
    ...
    while True:  # serve video forever
        ...
        if max_buffer_opt >= 0:
            max_buffer_size = min(max_buffer_size + max_buffer_opt, 60)
        else:
            max_buffer_size = max(max_buffer_size + max_buffer_opt, 4)
    ...
    return np.mean(reward_per_trace), np.mean(data_size_per_trace), np.mean(buffer_per_trace)