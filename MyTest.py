...
def main():
    ...
    # drag_time = np.random.choice(49)

    while True:  # serve video forever
        # the action is from the last decision
        # this is to make the framework similar to the real
        # if video_chunk_counter == drag_time:
        ...
        if first_step:
            pd.DataFrame(summary).to_csv(SUMMARY_DIR + '/summary_' + log_file.name.split('/')[2] + '.csv', \
                                         index=False)
            first_step = False
        else:
            pd.DataFrame(summary).to_csv(SUMMARY_DIR + '/summary_' + log_file.name.split('/')[2] + '.csv', \
